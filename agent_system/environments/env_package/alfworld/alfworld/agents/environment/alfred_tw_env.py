...
class AlfredTWEnv(object):
    ...
    def init_env_single(self, batch_size, game_file):
        """Initialize env with a single assigned game file for deterministic eval.

        Unlike init_env() which registers all game_files and relies on textworld's
        shuffled_cycle to select games on reset, this method registers only the
        specified game_file, ensuring the worker always plays exactly that game.
        """
        domain_randomization = False
        alfred_demangler = AlfredDemangler(shuffle=domain_randomization)
        wrappers = [alfred_demangler, AlfredInfos]
