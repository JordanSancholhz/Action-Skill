...
def get_top_n_product_from_keywords(
    ...
    if keywords[0] == '<r>':
        ...
        top_n_asins = search_engine.search(keywords, k=SEARCH_RETURN_N)
...
class PurePythonBM25Search:
    """Lightweight BM25 search engine that replaces Lucene/pyserini.
    Avoids spawning a JVM per worker, which exhausts OS threads under Ray.
    """
    def __init__(self, all_products):
        self._asins = [p['asin'] for p in all_products]
        corpus = []
        for p in all_products:
            options = p.get('options', {})
            option_texts = []
            for opt_name, opt_vals in options.items():
                option_texts.append(f'{opt_name}: {", ".join(opt_vals)}')
    ...
    def search(self, query, k=SEARCH_RETURN_N):
        tokens = query.lower().split()
        scores = self._bm25.get_scores(tokens)
        top_indices = scores.argsort()[::-1][:k]
        return [self._asins[i] for i in top_indices]


def init_search_engine(num_products=None, all_products=None):
    """Build a pure-Python BM25 search engine from the product list.
    The ``all_products`` argument is populated after ``load_products`` runs;
    ``num_products`` is accepted for backward-compatibility but ignored.
    """
    return PurePythonBM25Search(all_products)
