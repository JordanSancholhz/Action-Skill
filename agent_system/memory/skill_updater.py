...
1. OpenAI-compatible API (preferred):
    ...
    OPENAI_MODEL     – Model name (default: o3)
...
class SkillUpdater:
    def __init__(
        ...
        # Try OpenAI-compatible API first, then fall back to Azure.
        ...
        if openai_api_key and openai_base_url:
            ...
            print(f"[SkillUpdater] Using OpenAI-compatible API: {openai_base_url}, model={self.model}")
            ...
            self.model = "o3"
            print(f"[SkillUpdater] Using Azure OpenAI API: {azure_endpoint}, model={self.model}")
        else:
            raise EnvironmentError(
                "SkillUpdater requires either (OPENAI_API_KEY + OPENAI_BASE_URL) "
                "or (AZURE_OPENAI_API_KEY + AZURE_OPENAI_ENDPOINT) to be set."
            )
