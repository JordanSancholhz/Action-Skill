...
def assign_skill_type(row):
    ...
    if data_source.startswith('searchr1_'):
        data_source = data_source[len('searchr1_'):]
    ...
    if data_source == 'popqa':
        ...
        return 'compare'
    elif data_source in ('hotpotqa', '2wikimultihopqa', 'musique', 'bamboogle'):
...
def main():
    ...
    parser.add_argument('--output', type=str, default=None,
                        help='Explicit output parquet path (overrides --output_dir naming)')
    ...
    if 'env_kwargs' in df.columns:
        def add_skill_to_env(row):
            env_kwargs = dict(row['env_kwargs'] or {})
            env_kwargs['skill_type'] = row['skill_type']
            return env_kwargs
        df['env_kwargs'] = df.apply(add_skill_to_env, axis=1)
    ...
    if not sampled_parts:
        raise ValueError("No supported Search-QA samples remain after skill classification.")
