...
def normalize_ground_truth(value):
    """Return the Search environment's canonical ``{"target": [...]}`` shape."""
    if isinstance(value, dict):
        target = value.get("target", value.get("answers", []))
    else:
        target = value
    if target is None:
        target = []
    ...
    if isinstance(target, str):
        ...
        target = [str(target)]
    return {"target": [str(answer) for answer in target]}


def assign_skill_type(data_source, question):
    """Map Search-QA sources/questions to the bundled skill-bank categories."""
    ...
    if source in {"hotpotqa", "2wikimultihopqa", "musique", "bamboogle"}:
        if any(token in question_lower for token in ("which", "earlier", "later", "both", "same")):
            return "compare"
...
def process_single_row(row, current_split_name, row_index):
    ...
    if not isinstance(skill_type, str) or not skill_type.strip():
        skill_type = assign_skill_type(data_source_tagged, question)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_download_dir:
        for split in ["train", "test"]:
            ...
            try:
                if args.source_dir:
                    local_parquet_filepath = os.path.join(
                        os.path.abspath(os.path.expanduser(args.source_dir)),
                        parquet_filename,
                    )
                    if not os.path.isfile(local_parquet_filepath):
                        raise FileNotFoundError(
                            f"Expected local Search-QA file: {local_parquet_filepath}"
                        )
                    logger.info(f"Using local source file {local_parquet_filepath}")
                else:
                ...
                if max_samples is not None and len(df_raw) > max_samples:
                    df_raw = df_raw.sample(n=max_samples, random_state=args.seed).reset_index(drop=True)
                    logger.info(f"Sampled {len(df_raw)} rows for {split}")
    ...
    if args.hdfs_dir:
        try:
            # Import lazily so local-only preprocessing does not require the
            # complete Ray/verl training runtime.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source_dir",
        default=None,
        help="Optional directory containing already-downloaded train.parquet and test.parquet.",
    )
