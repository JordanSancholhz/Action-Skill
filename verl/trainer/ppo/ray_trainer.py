...
def compute_information_action_weights(
    ...
    z_clip: float = 3.0,
    competence_gate_coefficient: float = 4.0,
    eps: float = 1e-6,
):
    ...
    ``c p_q (1-p_q)`` gates the modulation by empirical task competence, where
    ``c`` is configurable for ablations. It vanishes for all-success and
    all-failure rollout groups where action-level credit assignment is not
    identifiable from outcomes.
    """
    ...
    if float(competence_gate_coefficient) < 0.0:
        raise ValueError(
            "competence gate coefficient must be non-negative, "
            f"got {competence_gate_coefficient}"
        )
    if float(eps) <= 0.0:
    ...
    for task_uid in dict.fromkeys(task_uids[valid_rows].tolist()):
        ...
        task_pass_rates.append(pass_rate)
        gate = (
            float(competence_gate_coefficient)
            * pass_rate
            * (1.0 - pass_rate)
        )
        idx = torch.as_tensor(idx_np, dtype=torch.long, device=device)
    ...
    metrics = {
        ...
        "action_ig/competence_gate_coefficient": float(
            competence_gate_coefficient
        ),
    ...
    def _update_easy_with_action_ig(
        ...
        if action_ig_active:
            action_weights, information_modulation, action_ig_metrics = compute_information_action_weights(
                ...
                competence_gate_coefficient=utilize_cfg.get(
                    'competence_gate_coefficient', 4.0
                ),
            )
