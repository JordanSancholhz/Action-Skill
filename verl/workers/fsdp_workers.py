...
class ActorRolloutRefWorker(Worker):
    ...
    def update_actor(self, data: DataProto):
        ...
        # In multi-turn mode, keep vLLM resident for every environment turn and
        # release it only at the rollout-to-backward boundary.
        if self.config.rollout.get("sleep_at_rollout_end", False):
            self.rollout_sharding_manager.sleep_inference_engine()
        self.rollout_sharding_manager.mark_inference_engine_weights_stale()
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def sleep_rollout_engine(self):
        """Release vLLM after the complete multi-turn rollout."""
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def compute_action_information(self, data: DataProto):
        """Return Skill/No-skill action JSD together with Skill old log-probs."""
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        data.meta_info["use_dynamic_bsz"] = self.config.rollout.log_prob_use_dynamic_bsz
        data.meta_info["temperature"] = self.config.rollout.temperature
        top_k = int(data.meta_info.get("action_information_top_k", 64))
        jsd_temperature = float(data.meta_info.get("action_information_temperature", 1.0))

        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            action_jsd, old_log_probs, entropys = self.actor.compute_action_information(
                data=data,
                top_k=top_k,
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_action_information", logger=logger)
        return output
