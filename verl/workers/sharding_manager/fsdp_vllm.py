...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __init__(
        ...
        # Keep the engine alive across multi-turn rollout calls when requested,
        # then release it once immediately before actor backward.
        self.sleep_at_rollout_end = bool(rollout_config.get("sleep_at_rollout_end", False))
        self._inference_engine_asleep = bool(rollout_config.free_cache_engine)
        self._inference_engine_weights_synced = False
    ...
    def __enter__(self):
        ...
        # Actor parameters cannot change during the 30 rollout turns.  Avoid
        # materializing and copying the full model for every individual turn.
        # Keep the legacy vLLM path unchanged: it needs a model sync for every
        # generation after offload.  The modern sleep API can safely reuse it.
        need_weight_sync = vllm_version in ("0.5.4", "0.6.3") or not self._inference_engine_weights_synced
        ...
        if vllm_version not in ("0.5.4", "0.6.3") and self.rollout_config.free_cache_engine and self._inference_engine_asleep:
        ...
        #             self.inference_engine.wake_up()
            ...
            if vllm_version in (
                ...
                if self.offload_param:
                    offload_fsdp_model_to_cpu(self.module)
                get_torch_device().empty_cache()
            self._inference_engine_weights_synced = True
        if vllm_version not in ("0.5.4", "0.6.3") and self.rollout_config.free_cache_engine and self._inference_engine_asleep:
            if "tags" in inspect.signature(self.inference_engine.wake_up).parameters:
            # if "tags" in inspect.signature(self.inference_engine.wake_up).parameters:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        #         self.inference_engine.offload_model_weights()
        #     else:
        #         # empty cache and synchronize before sleep to reduce peak GPU memory
        #         get_torch_device().empty_cache()
        #         get_torch_device().synchronize()
        #         self.inference_engine.sleep(level=2)
        if self.rollout_config.free_cache_engine and not self.sleep_at_rollout_end:
            self.sleep_inference_engine()
    ...
    def sleep_inference_engine(self):
        """Release vLLM once after a complete rollout, before actor backward."""
        if not self.rollout_config.free_cache_engine or self._inference_engine_asleep:
            return
        if vllm_version in (
            "0.5.4",
            "0.6.3",
            ...
            # Empty PyTorch's cache first so vLLM can unmap its CuMem allocations
            # without increasing the peak memory during the handoff to FSDP.
            get_torch_device().empty_cache()
            get_torch_device().synchronize()
            self.inference_engine.sleep(level=2)
        self._inference_engine_asleep = True
    def mark_inference_engine_weights_stale(self):
        """Require a fresh actor-weight sync before the next rollout."""
        self._inference_engine_weights_synced = False
